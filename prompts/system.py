...
def get_system_prompt(config: config.Config,
    user_memory: str | None = None, tools: List[Tool] | None = None
) -> str:
    parts = []

    # Identity and role
    parts.append(_get_identity_section())
    # Environmentparts.append(_get_environment_section(config))
...
def _get_environment_section(config: config.Config) -> str:
...
- **Formatting:** Respond in plain text only. Do not use Markdown. Do not wrap output in code blocks.
